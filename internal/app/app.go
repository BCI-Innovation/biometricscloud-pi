...
func New(cam camera.CameraStillReader, rs remote.RemoteService) (*App, error) {
	s := &App{
		cam:    cam,
		remote: rs,
		timer:  nil,
...
func (s *App) executeAtTick(dt time.Time) {
	fmt.Print(dt, "Tick")
	filePath, err := s.cam.TakeSnapshot()
	if err != nil {
		log.Fatal(err)
	}

	fmt.Println("Opening contents at file path:", filePath)
	bytes, err := os.ReadFile(filePath) // Load entire file into memory.
	if err != nil {
		log.Fatal(err)
	}

	fmt.Println("Submitting to remote service")
	err = s.remote.SubmitPhotoSample(s.cam.GetMetricID(), bytes, filePath)
	if err != nil {
		log.Fatal(err)
	}
